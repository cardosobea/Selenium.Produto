...
var produtosCadastrados = []; // Array para armazenar os códigos dos produtos cadastrados

// Define o idioma da tabela como português (Brasil)
tabela.lang = "pt-BR";
...
modal.btnSalvar.addEventListener('click', (e) =>{
   ...
   if (produtoJaCadastrado(produto.codigo)) {
        mostarAlerta("Esse código de produto já foi cadastrado!");
        return;
    }

    adicionarProdutoNaTabela(produto);
    produtosCadastrados.push(produto.codigo); // Adiciona o código do produto ao array de produtos cadastrados
    limparCampos();
    fecharModalProdutos();
});
...
function produtoJaCadastrado(codigo) {
    return produtosCadastrados.includes(codigo);
}
...
function adicionarProdutoNaTabela(produto) {
    ...
    tdNome.textContent = produto.nome.charAt(0).toUpperCase() + produto.nome.slice(1).toLowerCase(); //Converte a primeira letra em maiúscula e o restante da palavra em minúscula
